...
const CreateListingPage = () => {
  ...
  const handleSubmit = async () => {
    if (!validateForm()) {
      alert("Please fill in all required fields, including uploading an image(s).");
  ...
  const createItemListing = async () => {
    ...
    try {
      const token = await getAccessTokenSilently();
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/listings/create/item`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
